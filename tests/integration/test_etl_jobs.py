...
from noveum_api_client.api.etl_jobs.get_api_v1_etl_jobs import sync_detailed as get_api_v1_etl_jobs

API_KEY = os.getenv("NOVEUM_API_KEY", "******")
BASE_URL = os.getenv("NOVEUM_BASE_URL", "https://api.noveum.ai")

test_results: list[dict[str, Any]] = []
created_resources: dict[str, list[Any]] = {"jobs": []}
...
def test_list_jobs(low_level_client):
    print_section("TEST 1: List ETL Jobs")
    try:
        response = get_api_v1_etl_jobs(client=low_level_client, limit=10)
        passed = response.status_code == 200
        log_test("List ETL jobs", passed, f"Status: {response.status_code}")
    except Exception as e:
        log_test("List ETL jobs", False, f"Exception: {str(e)}")


def run_all_tests():
    print("\n" + "=" * 60)
    print("ETL JOBS API TESTS")
    print("=" * 60)
    ...
    test_list_jobs(low_level_client)
    ...
    try:
        ...
        with open(f"../../test_results/etl_jobs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "w") as f:
