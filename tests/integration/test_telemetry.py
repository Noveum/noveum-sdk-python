...
ORG_ID = os.getenv("NOVEUM_ORGANIZATION_ID", os.getenv("NOVEUM_ORG_ID", "NoveumSDK"))
...
@pytest.mark.skip(reason="Telemetry tests skipped - authentication/permissions issue")
def test_dashboard_metrics():
    print_section("TEST 1: Dashboard Metrics")
    response = get_api_telemetry_metrics_dashboard.sync_detailed(client=low_level_client, organization_id=ORG_ID)
...
def test_metrics():
    ...
    response = get_api_telemetry_metrics.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_metrics_with_trends():
    ...
    response = get_api_telemetry_metrics_with_trends.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_cost_by_provider():
    ...
    response = get_api_telemetry_metrics_cost_by_provider.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_cost_trends():
    ...
    response = get_api_telemetry_metrics_cost_trends.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_cost_trends_per_project():
    ...
    response = get_api_telemetry_metrics_cost_trends_per_project.sync_detailed(
        client=low_level_client, organization_id=ORG_ID
    )
    passed = response.status_code == 200
...
def test_latency_trends():
    ...
    response = get_api_telemetry_metrics_latency_trends.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_model_usage():
    ...
    response = get_api_telemetry_metrics_model_usage.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_error_rate():
    ...
    response = get_api_telemetry_metrics_error_rate.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_recent_errors():
    ...
    response = get_api_telemetry_metrics_recent_errors.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_requests_trends():
    ...
    response = get_api_telemetry_metrics_requests_trends.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_requests_trends_per_project():
    ...
    response = get_api_telemetry_metrics_requests_trends_per_project.sync_detailed(
        client=low_level_client, organization_id=ORG_ID
    )
    passed = response.status_code == 200
...
def test_slowest_requests():
    ...
    response = get_api_telemetry_metrics_slowest_requests.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_top_projects_api():
    ...
    response = get_api_telemetry_metrics_top_projects_api.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_top_projects_tokens():
    ...
    response = get_api_telemetry_metrics_top_projects_tokens.sync_detailed(
        client=low_level_client, organization_id=ORG_ID
    )
    passed = response.status_code == 200
...
def test_usage_trends():
    ...
    response = get_api_telemetry_metrics_usage_trends.sync_detailed(client=low_level_client, organization_id=ORG_ID)
    passed = response.status_code == 200
...
def test_usage_trends_per_project():
    ...
    response = get_api_telemetry_metrics_usage_trends_per_project.sync_detailed(
        client=low_level_client, organization_id=ORG_ID
    )
    passed = response.status_code == 200
